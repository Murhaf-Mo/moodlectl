...
from moodlectl.output.charts import _AVAILABLE as _CHARTS_AVAILABLE
...
def letter_grades(
    ...
    from moodlectl.features.analytics import get_grade_distribution
    from moodlectl.features.grades import compute_stats
    from moodlectl.output.charts import _bucket_grades, plot_letter_grade_bars
    from moodlectl.output.formatters import print_table
    ...
    buckets = _bucket_grades(grades, effective_max)
    total = len(grades)
    table_data = [
...
def summary(
    ...
    if grades:
        from moodlectl.output.charts import _bucket_grades
        auto_max = stats["max"] if stats["count"] else 100.0
        buckets = _bucket_grades(grades, auto_max)
        total = len(grades)
        print_table(
